...
func LogVariantAutoscalingStatus(ctx context.Context, vaName, namespace string, crClient client.Client) error {
	...
	fmt.Printf("Load Profile for VA: %s - Arrival Rate: %s, Avg Input Tokens: %s, Avg Output Tokens: %s\n",
		...
		variantAutoscaling.Status.Load.ArrivalRate,
		variantAutoscaling.Status.Load.AvgInputTokens, variantAutoscaling.Status.Load.AvgOutputTokens)
	...
	if len(variantAutoscaling.Status.DesiredOptimizedAllocs) > 0 {
		fmt.Printf("Desired Optimized Allocation for VA: %s - Replicas: %d, Accelerator: %s\n",
			variantAutoscaling.Name,
			variantAutoscaling.Status.DesiredOptimizedAllocs[0].NumReplicas,
			variantAutoscaling.Status.DesiredOptimizedAllocs[0].Accelerator)
	}
	return nil
}
