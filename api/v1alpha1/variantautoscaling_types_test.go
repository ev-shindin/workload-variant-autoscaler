...
func TestJSONRoundTrip(t *testing.T) {
	...
	if len(back.Status.DesiredOptimizedAllocs) == 0 {
		t.Fatalf("DesiredOptimizedAllocs should not be empty after unmarshal")
	}

	ot := orig.Status.DesiredOptimizedAllocs[0].LastRunTime.Time
	bt := back.Status.DesiredOptimizedAllocs[0].LastRunTime.Time
	if !ot.Equal(bt) {
		t.Fatalf("LastRunTime mismatch by instant: orig=%v back=%v", ot, bt)
	}

	back.Status.DesiredOptimizedAllocs[0].LastRunTime = orig.Status.DesiredOptimizedAllocs[0].LastRunTime
...
func TestOptimizedAllocLastRunTimeJSON(t *testing.T) {
	...
	if len(probe.Status.DesiredOptimizedAllocs) == 0 {
		t.Fatalf("expected at least one desiredOptimizedAlloc")
	}
	if probe.Status.DesiredOptimizedAllocs[0].LastRunTime == "" {
