...
func (in *VariantAutoscalingStatus) DeepCopyInto(out *VariantAutoscalingStatus) {
	*out = *in
	out.Load = in.Load
	if in.CurrentAllocs != nil {
		in, out := &in.CurrentAllocs, &out.CurrentAllocs
		*out = make([]Allocation, len(*in))
		copy(*out, *in)
	}
	if in.DesiredOptimizedAllocs != nil {
		in, out := &in.DesiredOptimizedAllocs, &out.DesiredOptimizedAllocs
		*out = make([]OptimizedAlloc, len(*in))
		for i := range *in {
			(*in)[i].DeepCopyInto(&(*out)[i])
		}
	}
