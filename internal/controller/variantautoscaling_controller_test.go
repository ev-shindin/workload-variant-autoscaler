...
%s`, freemiumModels)
		...
		It("should prepare active VAs for optimization", func() {
			...
			for _, updatedVa := range updateList.Items {
				...
				Expect(len(updatedVa.Status.CurrentAllocs)).To(BeNumerically(">", 0), fmt.Sprintf("CurrentAllocs should not be empty for %s after preparation", updatedVa.Name))
				if len(updatedVa.Status.CurrentAllocs) > 0 {
					Expect(updatedVa.Status.CurrentAllocs[0].Accelerator).To(Equal("A100"), fmt.Sprintf("Current Accelerator for %s should be \"A100\" after preparation", updatedVa.Name))
					Expect(updatedVa.Status.CurrentAllocs[0].NumReplicas).To(Equal(1), fmt.Sprintf("Current NumReplicas for %s should be 1 after preparation", updatedVa.Name))
				}
				// DesiredOptimizedAllocs may be empty initially after preparation
				if len(updatedVa.Status.DesiredOptimizedAllocs) > 0 {
					Expect(updatedVa.Status.DesiredOptimizedAllocs[0].Accelerator).NotTo(BeEmpty(), fmt.Sprintf("Desired Accelerator for %s should be set if DesiredOptimizedAllocs is not empty", updatedVa.Name))
				}
			}
		})
